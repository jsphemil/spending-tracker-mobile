...
- A PIN/passcode lock (already planned as a later feature, section 6)
  ...
  — **built as the biometric app lock in 3.0, see §5.23** (2026-09-11)
...
**Resize bugs — found and investigated 2026-09-09, fix deferred.**
Reported with screenshots: (1) with fewer accounts selected than the
widget's default grid height, a blank gap opens between the last
account row and the action buttons; (2) manually shrinking the widget
clips a row's middle instead of showing only whole rows. Root-caused
to the account list's `defaultWeight()`-stretched `LazyColumn` — extra
allocated height shows as a gap, and native list scrolling clips at
whatever pixel boundary the resize lands on, independent of row
height. The planned fix (auto-request an ideal size via
`AppWidgetManager.updateAppWidgetSize()` on config-save, plus render
only whole rows with a "+N more" hint) turned out to rely on an API
that **does not exist on Android** — checked directly against the real
SDK stubs (`android-36/android.jar`'s `AppWidgetManager` class) after
a compile failure, corrected rather than shipped as broken code. The
only real options going forward are best-effort
(`updateAppWidgetProviderInfo`, unreliable across launchers, not
guaranteed to resize an already-placed widget) or shrinking the card
to wrap-content (reintroduces the "resize handles trail past the
card" problem `fillMaxSize()` was originally written to avoid).
Deferred at the user's request; the attempted change was fully
...
> **Removed 2026-09-10, replaced by §5.21 Funds.** Everything below
> describes the feature as it shipped, kept for history. The `goals`
> table was dropped in migration `0015`, and its screens, form, service,
> actions, queries and tests were deleted. Nothing was migrated: a goal's
> target was a *net-worth threshold*, a fund's is a *purchase cost*, so
> converting "reach ₹50L net worth" into a ₹50L fund at ₹0 funded would
> have been actively misleading. The core limitation that made it worth
> replacing: every goal was scored against the same portfolio-wide net
> worth, so multiple goals were just multiple thresholds on one number
> and none could hold money independently.
...
the previous session ended revealed, and the toggle still works. Shipped
as versionCode 15.
...
- **Started 2026-09-05**, direct master-prompt request (not an Inbox
  ...
  independently-selectable** Android home-screen widgets alongside the
  existing **Accounts & Quick Add** widget (§5.11), which this work
  explicitly does **not** modify, redesign, replace, or refactor — it
  must keep its exact current look, behavior, data, refresh, config,
  resize behavior, and deep links, and must remain addable/usable for
  anyone who already has it placed. Building one widget at a time on a
  dedicated branch, each locally built and on-device tested before
  starting the next; merges to `master` only once all three are ready
  to ship together, per the now-standing local-build-first workflow
  (see [[feedback_verify_before_shipping_fixes]]).
- **New Widget A — Quick Add Transaction ✅ Built & Verified
  (2026-09-06).** Action-only, no financial data shown. Visual design
  is an exact transcription of the user's finalized Figma export (a
  smooth-cornered triangular composition of three gradient-filled
  petals, not a rectangular card) with three independently-tappable
  regions — Expense / Income / Transfer — around a central hub that
  shows the app's own live icon. Each tap opens the existing
  `app/transaction/new.tsx` flow via the same
  `spendingtracker://transaction/new?type=...` deep link the existing
  widget's own quick-add pills already use — no second transaction-entry
  implementation. Content is inscribed in a centered square
  (`min(width, height)`) regardless of the launcher's allocated box, so
  it never distorts into a non-square shape.
  - Android launcher shortcuts (Expense/Income/Transfer, same deep
    link) shipped alongside it via a new local config plugin
    (`plugins/withAppShortcuts.js`), since `android/` is regenerated by
    `expo prebuild` and can't be hand-edited.
  - **Three real bugs found and fixed during on-device testing**: (1)
    the hub icon never rendered — root-caused to combining
    `.padding()` with `.size()` on the same Glance modifier chain
    (the one element that didn't use the same fillMax/height/
    defaultWeight pattern as everything else); fixed by positioning it
    via sibling spacer boxes instead, confirmed via before/after
    device screenshots. (2) Tapping a second quick-add action right
    after another (e.g. Income right after Transfer) sometimes
    reopened the previous type — root cause: `MainActivity` is
    `launchMode="singleTask"`, so a rapid second deep link reuses the
    already-mounted `TransactionForm` instead of a fresh one, and its
    `useState(initialValues?.type ?? "expense")` only applies
    `initialValues` on first mount. Fixed with a `key` on
    `<TransactionForm>` derived from the params that identify "what am
    I creating," forcing a clean remount whenever they change. (3) A
    shortcut sometimes appearing to "add an icon" to the home screen
    instead of opening the app directly is not a bug — dragging a
...
- **New Widget C — Net Worth & Financial Composition. ❌ Dropped
  (2026-09-09).** Paused alongside Widget B on 2026-09-08, then
  explicitly dropped by the user on 2026-09-09 — not building it.
  Scope had been agreed but no code was ever written. Net worth in
  ...
  `analytics.tsx`'s `ASSET_BUCKETS`: **Liquid (Savings/Wallet)**,
  **Deposits (FD/RD)**, **Invested** — plus a **Liabilities** segment
  for credit card debt, in the same ring. This deliberately does *not*
  use the original request's own example categories ("Property",
  "Loans") since neither exists as an account type in this app
  (confirmed against `db/schema.ts`'s account-type enum: savings,
  investment, deposit, wallet, credit_card) — inventing them would
  violate the request's own "don't invent categories that don't exist"
  rule. **This also fulfills the previously-planned, never-started
  backlog item "Widget 2 (Portfolio Rings & Allocation)"** (§5.11) —
  same feature (net worth gauge + asset allocation donut, miniaturized
  for a widget), so it supersedes that item rather than being built
  twice. Whole-portfolio, base-currency, no config screen, same as
  Widget B.
- **Android launcher shortcuts.** Long-pressing the app icon offers
  Expense / Income / Transfer as static shortcuts
  (`ShortcutManager`/`shortcuts.xml`), using the same deep-link scheme
  as the widgets above — an additional entry point, not a widget.
- **Responsive sizing is a hard requirement for all three new
  widgets** — content must reflow (not just scale) across the
  launcher's actual allocated width/height, using Glance's
  `LocalSize.current` inside `provideContent`, the same
  size-responsiveness pattern already proven fixing the existing
  widget's account-list layout (§5.11's `LazyColumn` fix). The
  triangular Widget A's shape itself is generated as a size-aware
  drawn bitmap (rounded-path `Canvas`/`Paint`), not a static image
...
- **Origin (2026-09-11):** a paid Testers Community pass on the Play
  ...
  worked as intended across their devices. The report
  (`project-docs/product/erebor_feedback (1).pdf`) is eight enhancement
  suggestions. Two — Google Sign-in and email/password accounts with
  sync — were **declined** the same day: they cut against §3/§4's
  local-first, no-server decision, which the Dropbox whole-file backup
  already serves for cross-device continuity. The other six are this
  section, planned together as **3.0.0** on branch `v3/tester-feedback`,
  with the user's own addition, §5.23's biometric lock, going in first
  because it is the only native change.
- **Decisions taken up front, with the user:** the walkthrough is
  first-visit hint cards on each main screen plus "Replay the intro" in
  Settings — no spotlight/coach-marks overlay; feedback is a `mailto:`
  to the address the privacy policy already publishes, with version and
  device prefilled and never any financial data; the chart work is a
  smoothed net-worth trend, an earmarked-funds marker on the asset
  allocation ring and a new cumulative-spend chart (this month vs last)
  — not an income-vs-spending bar chart; Dashboard customisation is
  hide/show + reorder with Net worth pinned, plus choosing which
  shortcuts appear.
- **Data:** four columns on `settings` in one migration, `0017` —
  `app_lock_enabled`, `last_seen_version`, and two JSON text columns,
  `dashboard_layout` and `hints_seen`, each read only through a
  sanitiser that tolerates null, garbage and unknown ids. No backfill:
  a null `last_seen_version` on an install that has already completed
  onboarding is exactly the "just upgraded to 3.0" case.
- **What already existed and is extended rather than duplicated:** the
  4-step first-run intro and the header's ⓘ replay of its how-to copy
  (§5.13/§5.19), the three-question Help & Support page, Analytics'
  hand-drawn SVG charts, and the Dashboard's five fixed cards.
- **Features, in build order after §5.23:**
  1. **Help & Support → FAQ + Send feedback** — closes §5.15.
  2. **What's new** — a bundled changelog, a Settings screen, and a
     one-time sheet on the first launch after an update.
  3. **Walkthrough** — first-visit hint cards on seven screens, a replay
     route that also re-arms the hints, and the header's duplicated
     how-to copy retired in favour of the shared module.
  4. **Dashboard customisation** — a Settings screen with per-card
     eye toggles and ▲/▼ ordering; the Dashboard renders cards from the
     saved order with every calculation left where it is.
  5. **Analytics** — monotone-cubic smoothing (not Catmull-Rom, which
...
- **Added by the user 2026-09-11** alongside §5.22. This is the
  ...
  data, **not a login**, so it doesn't reintroduce the account model
  the testers suggested and the user declined.
- **Dependency:** `expo-local-authentication` (`~57.0.2`), the only new
  native module in 3.0 — hence a native rebuild, and hence it goes in
  first so every later phase tests on a binary that already has it.
  Its config plugin adds `USE_BIOMETRIC`/`USE_FINGERPRINT`.
- **Model:** nothing is stored by the app — no secret, no hash. The OS
  `BiometricPrompt` answers "is this the device owner?" and the root
  layout gates on the answer. `disableDeviceFallback` is left false so
  the device PIN/pattern is accepted when biometrics fail (a wet finger
  must not lock someone out of their finances) — on Android the module
  then requests `BIOMETRIC_WEAK | DEVICE_CREDENTIAL`, which is also why
  no custom cancel label is passed: the prompt ignores it whenever
  device credentials are allowed (verified in the module's Kotlin, not
  just the docs, which list the option as iOS-only).
- **Where it lives:** Settings → Privacy & Security → "Unlock with
  biometrics". Turning it on runs one prompt first and only persists on
  success, so it can never be enabled into a lock-out; with nothing
  enrolled the toggle is disabled with a "set up fingerprint or face
  unlock in your phone's settings first" line.
- **The gate** (`hooks/useAppLock.ts`, `components/LockScreen.tsx`,
  `app/_layout.tsx`): locked on cold start whenever enabled, re-locked
  when the app returns from the background after a fixed **30-second
  grace** (switching to the mail app to send feedback and back must not
  re-prompt). The lock screen renders *instead of* the navigator, not
  over it, so nothing underneath renders or can be captured; widget
  Quick Add and launcher shortcuts deep-link through the root layout,
  so they open after the unlock. `locked` starts true — "locked until
  proven otherwise" — because settings load asynchronously and an
  effect would paint one unlocked frame first; the undefined→true
  transition on a cold start keeps it locked, while false→true (the
  user just switched it on, so they are demonstrably inside the app)
  does not.
- **Decided:** the Accounts home-screen widget keeps showing balances
  while the app is locked — the home screen is already behind the
  phone's own lock — and the FAQ says so. No app-defined PIN, no
